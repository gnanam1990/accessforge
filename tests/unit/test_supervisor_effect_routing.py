...
def test_only_authenticated_original_binding_selects_effect_backend(
    ...
    monkeypatch.setattr(supervisor_sessions, "_live", live)
    for name, module in (
        (
            "baseline",
            baseline_effects if operation == "authorize_form" else baseline_effect_delivery,
        ),
        (
            "candidate",
            candidate_effects if operation == "authorize_form" else candidate_effect_delivery,
        ),
    ):
    ...
    if baseline == candidate:
        with pytest.raises(supervisor_sessions.Refused):
