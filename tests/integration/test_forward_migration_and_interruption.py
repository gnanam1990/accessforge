...
NEWEST = "0074_membership_invitation.sql"
...
def test_invitation_migration_preserves_membership_and_starts_empty(disposable: str) -> None:
    _apply_through(disposable, "0073_membership_revision.sql")
    user = str(uuid.uuid4())
    with connect(disposable) as conn:
        conn.execute("INSERT INTO workspace(id,name) VALUES (%s,'Invitation upgrade')", (WS,))
        conn.execute(
            "INSERT INTO app_user(id,email) VALUES (%s,%s)", (user, f"{user}@example.test")
        )
        conn.execute("SELECT set_config('accessforge.workspace_id',%s,true)", (WS,))
        conn.execute(
            "INSERT INTO workspace_membership(workspace_id,user_id,role,revision) "
            "VALUES (%s,%s,'OWNER',5)",
            (WS, user),
        )
        before = conn.execute("SELECT * FROM workspace_membership").fetchall()
    assert migrate(disposable) == [NEWEST]
    assert migrate(disposable) == []
    with connect(disposable) as conn:
        conn.execute("SELECT set_config('accessforge.workspace_id',%s,true)", (WS,))
        assert conn.execute("SELECT * FROM workspace_membership").fetchall() == before
        assert conn.execute("SELECT * FROM membership_invitation").fetchall() == []
        policy = conn.execute(
            "SELECT relrowsecurity,relforcerowsecurity FROM pg_class "
            "WHERE oid='membership_invitation'::regclass"
        ).fetchone()
        assert policy and policy["relrowsecurity"] and policy["relforcerowsecurity"]
...
def test_login_challenge_upgrade_preserves_users_without_creating_login_authority(
    ...
    assert migrate(disposable) == [
        "0071_github_login_challenge.sql",
        "0072_github_user_identity.sql",
        "0073_membership_revision.sql",
        NEWEST,
    ]
    assert migrate(disposable) == []
    with connect(disposable) as conn:
...
def test_identity_upgrade_preserves_unbound_sessions(disposable: str) -> None:
    _apply_through(disposable, "0071_github_login_challenge.sql")
    user_id, session_id = str(uuid.uuid4()), str(uuid.uuid4())
    with connect(disposable) as conn:
        conn.execute("INSERT INTO app_user(id,email) VALUES(%s,'prior@example.test')", (user_id,))
        conn.execute(
            "INSERT INTO user_session(id,user_id,token_hash,csrf_token_hash,expires_at) "
            "VALUES(%s,%s,repeat('a',64),repeat('b',64),now()+interval '1 hour')",
            (session_id, user_id),
        )
        before = conn.execute("SELECT * FROM user_session").fetchone()
    assert migrate(disposable) == [
        "0072_github_user_identity.sql",
        "0073_membership_revision.sql",
        NEWEST,
    ...
    with connect(disposable) as conn:
        after = conn.execute("SELECT * FROM user_session").fetchone()
        assert after is not None and after.pop("github_subject") is None
        assert after == before
        assert conn.execute("SELECT * FROM github_user_identity").fetchall() == []


def test_membership_revision_upgrade_preserves_roles_and_revocations(disposable: str) -> None:
    _apply_through(disposable, "0072_github_user_identity.sql")
    workspace = str(uuid.uuid4())
    with connect(disposable) as conn:
        conn.execute("INSERT INTO workspace(id,name) VALUES (%s,'Prior memberships')", (workspace,))
        conn.execute("SELECT set_config('accessforge.workspace_id',%s,true)", (workspace,))
        for role in ("OWNER", "MAINTAINER", "REVIEWER", "VIEWER"):
            user = str(uuid.uuid4())
            conn.execute(
                "INSERT INTO app_user(id,email) VALUES (%s,%s)", (user, f"{user}@example.test")
            )
            conn.execute(
                ...
                (workspace, user, role, role == "VIEWER"),
            )
        before = conn.execute("SELECT * FROM workspace_membership ORDER BY user_id").fetchall()
    assert migrate(disposable) == ["0073_membership_revision.sql", NEWEST]
    assert migrate(disposable) == []
    with connect(disposable) as conn:
        conn.execute("SELECT set_config('accessforge.workspace_id',%s,true)", (workspace,))
        after = conn.execute("SELECT * FROM workspace_membership ORDER BY user_id").fetchall()
        assert len(after) == len(before) == 4
        for original, updated in zip(before, after, strict=True):
            assert updated.pop("revision") == 1
            assert updated == original
        with pytest.raises(psycopg.IntegrityError), conn.transaction():
            conn.execute(
                "UPDATE workspace_membership SET revision=0 WHERE workspace_id=%s", (workspace,)
            )
...
def test_github_preview_upgrade_does_not_invent_publication_intent(disposable: str) -> None:
    ...
    migrate(disposable)
    with connect(disposable) as conn:
        assert conn.execute("SELECT id FROM github_publication_preview").fetchall() == []
        row = conn.execute(
            "SELECT relrowsecurity,relforcerowsecurity FROM pg_class "
...
def test_publication_receipt_upgrade_does_not_invent_creation_confirmation(disposable: str) -> None:
    _apply_through(disposable, "0068_github_publication_intent.sql")
    _seed_released_lease(disposable, reason="OPERATOR_RESET")
    intent_id = str(uuid.uuid4())
    with connect(disposable) as conn:
        conn.execute(
            "INSERT INTO github_publication_intent(id,workspace_id,app_id,repository_id,"
            "run_id,preview_id,approval_id,preview_digest) "
            "VALUES(%s,%s,7,13,%s,%s,%s,repeat('a',64))",
    ...
    with connect(disposable) as conn:
        ...
        row = conn.execute(
            "SELECT relrowsecurity,relforcerowsecurity FROM pg_class "
            "WHERE oid='github_publication_receipt'::regclass"
        ).fetchone()
        assert row is not None and row["relrowsecurity"] and row["relforcerowsecurity"]
        assert conn.execute(
            "SELECT count(*) AS n FROM pg_constraint "
            "WHERE conrelid='github_publication_intent'::regclass AND contype='u' "
            "AND pg_get_constraintdef(oid)='UNIQUE (id, workspace_id)'"
        ).fetchone() == {"n": 1}
        with pytest.raises(psycopg.IntegrityError), conn.transaction():
            conn.execute("DELETE FROM github_publication_intent WHERE id=%s", (intent_id,))


def test_binding_upgrade_does_not_invent_repository_authority(disposable: str) -> None:
    _apply_through(disposable, "0065_github_webhook_inbox.sql")
    migrate(disposable)
    with connect(disposable) as conn:
        assert conn.execute("SELECT id FROM github_repository_binding").fetchall() == []
        row = conn.execute(
            "SELECT relrowsecurity,relforcerowsecurity FROM pg_class "
            "WHERE oid='github_repository_binding'::regclass"
        ).fetchone()
        assert row is not None and row["relrowsecurity"] and row["relforcerowsecurity"]
...
def test_dispatch_migration_does_not_invent_historical_machine_credentials(disposable: str) -> None:
    ...
    assert migrate(disposable) == [
        ...
        "0051_fixture_setup_reservation.sql",
        "0052_mandatory_fixture_admission.sql",
        "0053_fixture_setup_artifact.sql",
