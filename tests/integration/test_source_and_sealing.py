...
def test_requested_revision_must_match_the_checkout_being_hashed(repo: Path) -> None:
    first = _git(repo, "rev-parse", "HEAD")
    (repo / "app.py").write_text("print('v2')\n", encoding="utf-8")
    _git(repo, "commit", "-qam", "second")
    with pytest.raises(source_intake.ReproducibilityRefused, match="not the checked-out commit"):
        source_intake.resolve_source(repo, revision=first)


def test_ignored_source_bytes_cannot_be_described_as_clean_commit(repo: Path) -> None:
    (repo / ".gitignore").write_text("ignored.txt\n", encoding="utf-8")
    _git(repo, "add", ".gitignore")
    _git(repo, "commit", "-qm", "ignore generated input")
    before = source_intake.resolve_source(repo)
    (repo / "ignored.txt").write_text("generated bytes", encoding="utf-8")
    observed = source_intake.resolve_source(repo)
    assert observed.tree_digest != before.tree_digest
    assert observed.dirty
    assert "ignored.txt" in observed.dirty_paths
    with pytest.raises(source_intake.ReproducibilityRefused):
        source_intake.assert_reproducible(observed)


def test_source_status_preserves_newlines_quotes_and_rename_paths(repo: Path) -> None:
    destination = 'renamed\n"app".py'
    _git(repo, "mv", "app.py", destination)
    identity = source_intake.resolve_source(repo)
    assert set(identity.dirty_paths) == {"app.py", destination}
    assert identity.dirty


def test_source_observation_does_not_execute_repository_fsmonitor(repo: Path) -> None:
    hook = repo / "fsmonitor-hook"
    hook.write_text("#!/bin/sh\ntouch fsmonitor-called\nprintf '\\0'\n", encoding="utf-8")
    hook.chmod(0o700)
    _git(repo, "config", "core.fsmonitor", str(hook))
    source_intake.resolve_source(repo)
    assert not (repo / "fsmonitor-called").exists()


def test_offline_build_observation_measures_bytes_without_deployment_claim(
    repo: Path, tmp_path: Path, capsys: pytest.CaptureFixture[str]
) -> None:
    import hashlib
    import json

    from accessforge_persistence.build_observation import main

    artifact = tmp_path / "actual-build.bin"
    artifact.write_bytes(b"observed artifact")
    assert main(["--repo", str(repo), "--artifact", str(artifact)]) == 0
    payload = json.loads(capsys.readouterr().out)
    assert payload["commitSha"] == _git(repo, "rev-parse", "HEAD")
    assert payload["treeDigest"] == source_intake.resolve_source(repo).tree_digest
    assert payload["artifactDigest"] == hashlib.sha256(b"observed artifact").hexdigest()
    assert payload["identityObservable"] is False
...
def test_build_observation_refuses_an_artifact_inside_source(repo: Path) -> None:
    from accessforge_persistence.build_observation import observe_build

    with pytest.raises(source_intake.SourceIntakeError, match="outside"):
        observe_build(repo, repo / "app.py")


def test_source_change_during_observation_is_refused(
    repo: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    original = source_intake._hash_tree

    def change_after_hash(root: Path) -> tuple[str, int]:
        result = original(root)
        (root / "app.py").write_text("changed while measuring", encoding="utf-8")
        return result

    monkeypatch.setattr(source_intake, "_hash_tree", change_after_hash)
    with pytest.raises(source_intake.ReproducibilityRefused, match="changed during observation"):
        source_intake.resolve_source(repo)
