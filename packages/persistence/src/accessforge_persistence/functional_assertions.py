...
def receipt(
    ...
    row = conn.execute(
        ...
        "FROM candidate_run_binding b JOIN sealed_manifest s "
        "ON s.id=b.sealed_manifest_id AND s.run_id=b.run_id AND s.workspace_id=b.workspace_id "
        "JOIN run r ON r.id=b.run_id AND r.workspace_id=b.workspace_id "
        "JOIN candidate_reader_lease c ON c.run_id=b.run_id AND c.workspace_id=b.workspace_id "
