...
CREATE TABLE candidate_artifact_observation (
 id UUID PRIMARY KEY,
...
ALTER TABLE candidate_artifact_observation ENABLE ROW LEVEL SECURITY;
ALTER TABLE candidate_artifact_observation FORCE ROW LEVEL SECURITY;
CREATE POLICY workspace_isolation ON candidate_artifact_observation
 USING(workspace_id=current_workspace_id()) WITH CHECK(workspace_id=current_workspace_id());
CREATE FUNCTION guard_candidate_artifact_observation() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
 IF TG_OP='DELETE' AND NOT EXISTS(
   SELECT 1 FROM candidate_regression_attempt WHERE id=OLD.regression_attempt_id
 ) THEN RETURN OLD; END IF;
 IF TG_OP='INSERT' AND EXISTS(
   SELECT 1 FROM candidate_regression_attempt a JOIN candidate_endpoint e ON e.attempt_id=a.id
...
END;
$$;
CREATE TRIGGER candidate_artifact_observation_guard
 BEFORE INSERT OR UPDATE OR DELETE ON candidate_artifact_observation
 FOR EACH ROW EXECUTE FUNCTION guard_candidate_artifact_observation();
