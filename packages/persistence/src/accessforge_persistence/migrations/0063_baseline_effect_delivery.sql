...
CREATE TABLE baseline_action_effect_permit (
 id UUID PRIMARY KEY,
...
ALTER TABLE baseline_action_effect_permit ENABLE ROW LEVEL SECURITY;
ALTER TABLE baseline_action_effect_permit FORCE ROW LEVEL SECURITY;
CREATE POLICY workspace_isolation ON baseline_action_effect_permit
 USING(workspace_id=current_workspace_id()) WITH CHECK(workspace_id=current_workspace_id());
CREATE FUNCTION guard_baseline_action_effect_permit() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
 IF TG_OP='DELETE' AND (NOT EXISTS(SELECT 1 FROM workspace WHERE id=OLD.workspace_id)
   ...
   OR NOT EXISTS(SELECT 1 FROM baseline_regression_attempt WHERE id=OLD.regression_attempt_id))
...
END;
$$;
CREATE TRIGGER baseline_action_effect_permit_guard
 BEFORE INSERT OR UPDATE OR DELETE ON baseline_action_effect_permit
 FOR EACH ROW EXECUTE FUNCTION guard_baseline_action_effect_permit();

-- A consumed permission without a retained response is unresolved, never safe to replay.
ALTER TABLE baseline_action_effect_permit ADD CONSTRAINT baseline_effect_permit_id_workspace_unique
 UNIQUE(id,workspace_id);
CREATE TABLE baseline_effect_delivery (
 ...
 FOREIGN KEY(permit_id,workspace_id) REFERENCES baseline_action_effect_permit(id,workspace_id)
  ON DELETE CASCADE
);
ALTER TABLE baseline_effect_delivery ENABLE ROW LEVEL SECURITY;
ALTER TABLE baseline_effect_delivery FORCE ROW LEVEL SECURITY;
CREATE POLICY workspace_isolation ON baseline_effect_delivery
 USING(workspace_id=current_workspace_id()) WITH CHECK(workspace_id=current_workspace_id());
CREATE FUNCTION guard_baseline_effect_delivery() RETURNS trigger LANGUAGE plpgsql AS $$
BEGIN
 IF TG_OP='DELETE' AND (NOT EXISTS(SELECT 1 FROM workspace WHERE id=OLD.workspace_id)
   OR NOT EXISTS(SELECT 1 FROM baseline_action_effect_permit WHERE id=OLD.permit_id))
 THEN RETURN OLD; END IF;
 IF TG_OP='INSERT' AND NEW.response_digest IS NULL AND EXISTS(
   SELECT 1 FROM baseline_action_effect_permit WHERE id=NEW.permit_id
...
END;
$$;
CREATE TRIGGER baseline_effect_delivery_guard BEFORE INSERT OR UPDATE OR DELETE
 ON baseline_effect_delivery FOR EACH ROW EXECUTE FUNCTION guard_baseline_effect_delivery();
