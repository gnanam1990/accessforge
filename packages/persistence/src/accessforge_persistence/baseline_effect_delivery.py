...
from . import baseline_effects as permits
...
from . import runners, supervisor_sessions
...
def _live(
    ...
    *,
    ...
    stored = conn.execute(
        "SELECT * FROM baseline_action_effect_permit WHERE id=%s", (permit_id,)
    ).fetchone()
    ...
    locked = conn.execute(
        "SELECT * FROM baseline_action_effect_permit WHERE id=%s FOR UPDATE",
        (permit_id,),
    ).fetchone()
...
def begin(
    ...
    *,
    ...
    binding = conn.execute(
        "SELECT run_id FROM baseline_session_binding WHERE regression_attempt_id=%s",
        (claim.attempt_id,),
    ).fetchone()
    if binding is None:
        raise Refused("baseline effects require an original bound session; no preview")
    eligible = conn.execute(
        "SELECT p.id FROM baseline_action_effect_permit p "
    ...
    conn.execute(
        "UPDATE baseline_action_effect_permit SET consumed_at=clock_timestamp() WHERE id=%s",
        (identifier,),
    )
    conn.execute(
        "INSERT INTO baseline_effect_delivery(permit_id,workspace_id,request_digest) "
        "VALUES(%s,%s,%s)",
...
def check(
    ...
    *,
    ...
    delivery = conn.execute(
        "SELECT * FROM baseline_effect_delivery WHERE permit_id=%s FOR UPDATE", (permit["id"],)
    ).fetchone()
...
def retain_response(
    ...
    *,
    ...
    conn.execute(
        "UPDATE baseline_effect_delivery SET response_digest=%s,response_status=%s,"
...
def assert_action_result(conn: psycopg.Connection[Any], *, action_id: str) -> None:
    ...
    unresolved = conn.execute(
        "SELECT 1 FROM baseline_action_effect_permit p "
        "LEFT JOIN baseline_effect_delivery d ON d.permit_id=p.id "
...
def status(
    ...
    *,
    ...
    permit = conn.execute(
        "SELECT * FROM baseline_action_effect_permit WHERE action_id=%s", (action_id,)
    ).fetchone()
    if permit is None:
        raise Refused("original form permission unavailable")
    view = permits._view(permit)
    delivery = conn.execute(
        "SELECT response_digest FROM baseline_effect_delivery WHERE permit_id=%s", (permit["id"],)
    ).fetchone()
