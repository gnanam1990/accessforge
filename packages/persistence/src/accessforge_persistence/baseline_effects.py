...
from . import baseline_observations, supervisor_sessions
...
def authorize_form(
    ...
    *,
    ...
    binding = conn.execute(
        "SELECT b.*,e.expires_at,e.plan FROM baseline_session_binding b JOIN baseline_endpoint e "
    ...
    prior = conn.execute(
        "SELECT * FROM baseline_action_effect_permit WHERE action_id=%s",
        (action_id,),
    ).fetchone()
    ...
    try:
        ...
        measurement = baseline_observations.for_runtime_preflight(
            conn,
            session=session,
    ...
    except (KeyError, TypeError, ValueError, baseline_observations.Refused) as exc:
    ...
    identifier = str(uuid5(NAMESPACE_URL, "accessforge:baseline-action-effect:" + fingerprint))
    stored = conn.execute(
        "INSERT INTO baseline_action_effect_permit "
...
def _view(row: dict[str, Any]) -> dict[str, Any]:
    ...
    if fingerprint != row["grant_digest"] or str(row["id"]) != str(
        uuid5(NAMESPACE_URL, "accessforge:baseline-action-effect:" + fingerprint)
    ):
