...
from accessforge_persistence import diagnoses, workspace_connection
...
async def deliver(
    ...
    *,
    ...
    # No database locks span the provider call. Re-read physical source and retained evidence;
    # revocation, deletion or changes during model work cannot inherit the earlier read's authority.
    with workspace_connection(database_url, workspace_id) as conn:
        diagnoses.authorize(conn, workspace_id, requested_by)
        # Serialize the final read/write before taking artifact locks; do not upgrade two shared
        # run locks after concurrent deliveries have started waiting on each other's artifacts.
        conn.execute(
            "SELECT id FROM run WHERE id=%s AND workspace_id=%s FOR UPDATE", (run_id, workspace_id)
        )
        after = prepare(
            conn,
            store,
        ...
        # Concurrent duplicate delivery may have committed while the model was in flight. Do not
        # overwrite it or let nondeterministic model wording produce a second occurrence.
        existing = diagnoses.by_operation(
            conn, workspace_id=workspace_id, operation_id=operation_id
        )
        if existing is not None:
            if existing["requestDigest"] != identity:
                raise Refused("concurrent diagnosis operation has different inputs")
            return existing
        return diagnoses.retain(
            conn,
            workspace_id=workspace_id,
            run_id=run_id,
            requested_by=requested_by,
