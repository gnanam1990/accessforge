...
@router.post("/supervisor-sessions/{session_id}/actions/{action_id}/form-effect-permit")
def authorize_supervisor_form_effect(
    workspace_id: str,
    session_id: str,
    action_id: str,
    request: Request,
    ...
    try:
        ...
        raise ProblemDetail(
            ProblemCode.PERMISSION_DENIED, "form effect permission refused"
        ) from None
    response.headers["Cache-Control"] = "no-store"
    return result


@router.post("/supervisor-sessions/{session_id}/actions/{action_id}/preflight")
def retain_supervisor_runtime_preflight(
    workspace_id: str,
    session_id: str,
    action_id: str,
    request: Request,
    ...
    if credential is None or len(request.headers.getlist("authorization")) != 1:
        raise ProblemDetail(ProblemCode.NOT_AUTHENTICATED, "supervisor session unavailable")
    try:
        result = supervisor_sessions.retain_runtime_preflight(
            conn,
            workspace_id=workspace_id,
            session_id=session_id,
            token=credential.credentials,
        ...
        raise ProblemDetail(
            ProblemCode.PERMISSION_DENIED, "runtime preflight is not admitted"
        ) from None
    response.headers["Cache-Control"] = "no-store"
    return result
