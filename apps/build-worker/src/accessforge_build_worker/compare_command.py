"""Explicit read-only operator export of an original-source comparison to stdout."""
...
def main(argv: list[str] | None = None) -> None:
    ...
    if not args.include_source:
        parser.error("--include-source is required before source can be emitted to stdout")
    fence = Event()
    ...
    try:
        ...
        print(json.dumps({"status": "COMPARISON_UNAVAILABLE", "errorType": type(exc).__name__}))
