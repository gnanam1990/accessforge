...
describe('build and manifest preparation', () => {
  ...
  it('locks observed inputs and reuses identity after an unknown registration response', async () => {
    ...
    expect(screen.getByLabelText('Offline observation JSON')).toBeDisabled()
    await user.click(screen.getByRole('button', { name: 'Retry same build registration' }))
    await screen.findByRole('heading', { name: 'Build identity recorded' })
    expect(writes[0]?.body).toEqual(writes[1]?.body)
    expect(writes[0]?.headers.get('idempotency-key')).toBe(writes[1]?.headers.get('idempotency-key'))
  })
  it('imports into a draft without writing, preserves filenames, and resets target confirmation', async () => {
    const { writes } = setup()
    const user = await fillBuild()
    await user.click(screen.getByLabelText(/target deployment exposes/))
    await user.click(screen.getByText('Import offline observation JSON'))
    const dirtyPaths = [' space ', 'line\nbreak']
    fill('Offline observation JSON', JSON.stringify({ commitSha: 'd'.repeat(40), treeDigest: 'e'.repeat(64),
      artifactDigest: 'f'.repeat(64), requestedRevision: 'HEAD', dirtyPaths, dirty: true, identityObservable: false }))
    await user.click(screen.getByRole('button', { name: 'Load observation into draft' }))
    expect(screen.getByRole('status')).toHaveTextContent('Nothing has been sent')
    expect(writes).toEqual([])
    expect(screen.getByLabelText(/target deployment exposes/)).not.toBeChecked()
    expect(screen.getByLabelText(/Source commit SHA/)).toHaveValue('d'.repeat(40))
    await user.click(screen.getByRole('button', { name: 'Record observed build' }))
    await screen.findByRole('heading', { name: 'Build identity recorded' })
    expect(writes[0]?.body).toMatchObject({ dirtyPaths, dirty: true, identityObservable: false })
  })
  it('refuses invalid import without overwriting the draft or sending a write', async () => {
    const { writes } = setup()
    const user = await fillBuild()
    await user.click(screen.getByText('Import offline observation JSON'))
    fill('Offline observation JSON', '{')
    await user.click(screen.getByRole('button', { name: 'Load observation into draft' }))
    expect(await screen.findByRole('alert')).toHaveTextContent('Paste valid JSON')
    expect(screen.getByLabelText('Offline observation JSON')).toHaveAttribute('aria-invalid', 'true')
    expect(screen.getByLabelText(/Source commit SHA/)).toHaveValue('a'.repeat(40))
    expect(writes).toEqual([])
  })
